...
**Telegram-приложение через DPI надёжно не обходится.** Оно ходит к дата-центрам по IP (MTProto), и десинхронизация на нём не срабатывает. Для Telegram используйте встроенный **MTProto-прокси** (Настройки → Данные и память → Прокси) или VPN — Dash Connect его через DPI не гарантирует.
