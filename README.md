...
> **Лицензии.** Код в `mobile/` — производная работа от ByeDPIAndroid и распространяется под
> **GPL-3.0**: полный текст в [`mobile/LICENSE`](mobile/LICENSE), разбор происхождения каждого
> компонента — в [`mobile/NOTICE.md`](mobile/NOTICE.md). Десктопная часть лицензией пока не
> закрыта отдельно.
