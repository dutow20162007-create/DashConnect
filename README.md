...
- **Telegram работает сам после подключения.** При нажатии **Подключить** прога локально поднимает **WebSocket-мост** (open-source `tg-ws-proxy` от Flowseal): трафик Telegram оборачивается в WebSocket/TLS к **собственным дата-центрам Telegram** — для DPI это обычный HTTPS, без чужого сервера. Один раз подтверждаешь «Подключить прокси» в самом Telegram — дальше он подключается автоматически при каждом запуске. Если мост недоступен — резервно поднимает **Cloudflare WARP** (`warp-plus`) или находит рабочий MTProto-прокси.
...
- Telegram подключается к мосту через `tg://proxy?server=127.0.0.1&port=1443&secret=…`. Секрет постоянный (генерируется один раз и хранится в конфиге), поэтому при первом подключении достаточно один раз нажать в Telegram «Подключить прокси» — дальше он помнит прокси и подключается сам при каждом запуске.
- Кнопка **«Настроить Telegram»** (Настройки) поднимает мост и открывает эту ссылку вручную. Если мост почему-то не поднялся (нет `tg-ws-proxy.exe`, занят порт) — резервно включается **Cloudflare WARP** (каскад scan → warp-in-warp → Psiphon), а если и он заблокирован — находит рабочий **MTProto-прокси** из авто-обновляемых списков. Мост и WARP живут, пока запущен Dash Connect.
