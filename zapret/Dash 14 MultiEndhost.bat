...
--filter-tcp=80,443 --hostlist-auto="%LISTS%list-auto.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=multisplit --dpi-desync-split-pos=midsld,endhost-1 --dpi-desync-repeats=6 --dpi-desync-fake-http="%BIN%tls_clienthello_max_ru.bin" --new ^
...
--filter-tcp=80,443,8443 --ipset="%LISTS%ipset-all.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=multisplit --dpi-desync-split-pos=midsld,endhost-1 --dpi-desync-repeats=6 --dpi-desync-fake-http="%BIN%tls_clienthello_max_ru.bin" --new ^
