...
class ByeDpiProxy {
    ...
    /**
     * Create + bind the SOCKS listener only, returning the fd (>= 0) or -1 on failure. Split out from
     * [startProxy] so the VpnService can CONFIRM the proxy is actually bound BEFORE it raises the TUN —
     * otherwise a failed bind (e.g. a bad strategy arg) leaves the TUN up over a dead backend and
     * black-holes every packet.
     */
    suspend fun createProxy(preferences: ByeDpiProxyPreferences): Int = createSocket(preferences)

    /** Run the event loop on the already-created socket. Blocks until the proxy stops. */
    suspend fun runProxy(): Int {
        val f = mutex.withLock { fd }
        if (f < 0) throw IllegalStateException("Proxy socket not created")
